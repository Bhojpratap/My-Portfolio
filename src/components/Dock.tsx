...
const DOCK_ITEMS = [
  ...
  { id: "resume", icon: Icons.FileText, label: "Resume", href: "/Bhojpratap Sahu Software Developer Resume.pdf", external: true },
];
...
export default function Dock() {
  ...
  return (
    ...
    className="fixed  bottom-3 left-1/2 -translate-x-1/2 z-50 flex h-auto items-end gap-2 sm:bottom-6 sm:h-14 sm:gap-3 md:gap-4 md:bottom-8 md:h-16 md:pb-3 md:px-4 rounded-2xl border border-white/10 bg-white/5 p-2 sm:px-3 backdrop-blur-md overflow-x-auto md:overflow-visible"
    >
      {DOCK_ITEMS.map((item) => (
...
function DockIcon({
  ...
  // reduce magnification on small screens
  const widthSync = useTransform(distance, [-150, 0, 150], isSmall ? [36, 50, 36] : [40, 80, 40]);
  ...
  return (
      ...
      <motion.div
        ref={ref}
        style={{ width, minWidth: isSmall ? 36 : 40, maxWidth: isSmall ? 50 : 80 }}
        className="aspect-square md:w-10 rounded-full bg-gray-700/50 border border-white/10 flex items-center justify-center hover:bg-gray-600/80 transition-colors group relative"
      >
         <item.icon className="w-1/3 h-1/3 sm:w-1/3 sm:h-1/3 md:w-1/2 md:h-1/2 text-gray-200 group-hover:text-white" />
         
         {/* Tooltip (hidden on small screens) */}
         <span className="hidden md:block absolute -top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 md:group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-white/10">
            {item.label}
         </span>
      </motion.div>
    </Link>
